...
pub fn get_network_data(network_interface: String) -> HashMap<String, String> {
    let proc_net_dev = fs::read_to_string("/proc/net/dev");

    return match proc_net_dev {
        Ok(proc_net_dev) => parse_net_dev(proc_net_dev, network_interface),
        Err(_) => HashMap::new()
    }
}

fn parse_net_dev(proc_meminfo: String, network_interface: String) -> HashMap<String, String> {
    let mut values = HashMap::<String, String>::new();

    // enp6s0: 1436460811 1044283    0    0    0     0          0      4504 24663248  226471    0    0    0     0       0          0
    let re = Regex::new(&format!(r"{}:\s+([^\s]+)\s+[^\s]+\s+[^\s]+\s+[^\s]+\s+[^\s]+\s+[^\s]+\s+[^\s]+\s+[^\s]+\s+([^\s]+)", regex::escape(&network_interface))).unwrap();
